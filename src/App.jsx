...
function LoanRow({ loan, onOpen, onNotes, showDocsColumns }) {
  ...
  return (
    <tr onClick={() => onOpen(loan)}
      ...
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.muted, whiteSpace: "nowrap" }}>
        {loan.lastTouch ? fmtShort(loan.lastTouch) : "—"}
      </td>
      ...
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.muted, whiteSpace: "nowrap" }}>{loan.ariveLoanNum || "—"}</td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{loan.lender || "—"}</td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.muted, whiteSpace: "nowrap" }}>{loan.lenderLoanNum || "—"}</td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{loan.lo || "—"}</td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{loan.processor || "—"}</td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{loan.mortgageType || "—"}</td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{loan.loanType || "—"}</td>
      <td style={{ padding: "10px 12px", whiteSpace: "nowrap" }}><MilestoneBadge value={loan.milestone} small /></td>
      <td style={{ padding: "10px 12px", whiteSpace: "nowrap" }}><DaysChip dateStr={loan.estClosingDate} /></td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{fmtShort(loan.estClosingDate)}</td>
      <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text }}>
        {loan.lockExpiration ? fmtShort(loan.lockExpiration.slice(0,10)) : "—"}
      </td>
      <td style={{ padding: "10px 12px", fontSize: 12, whiteSpace: "nowrap", color: loan.toleranceCures ? "#e65100" : NXT.muted }}>
        {loan.toleranceCures || "—"}
      </td>
      {showDocsColumns && (
        <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{loan.docsSignedDate ? fmtShort(loan.docsSignedDate) : "—"}</td>
      )}
      {showDocsColumns && (
        <td style={{ padding: "10px 12px", fontSize: 12, color: NXT.text, whiteSpace: "nowrap" }}>{loan.loanFundedDate ? fmtDate(loan.loanFundedDate) : "—"}</td>
      )}
      ...
      <td style={{ padding: "10px 12px", fontSize: 12,
        color: (!loan.closer || loan.closer === "Unassigned") ? NXT.muted : NXT.text, whiteSpace: "nowrap" }}>
...
function PipelineTable({ loans, onOpen, onNotes, emptyMsg, showDocsColumns, sortCol, sortDir, onSort }) {
  ...
  const baseHeaders = ["Last Touch","Borrower","Arive #","Lender","Lender Loan #","LO","Processor","Mortgage Type","Loan Purpose","Milestone","Days","Close","Lock Exp","Cure","Notes","Closer"];
  const headers = showDocsColumns
    ? ["Last Touch","Borrower","Arive #","Lender","Lender Loan #","LO","Processor","Mortgage Type","Loan Purpose","Milestone","Days","Close","Lock Exp","Cure","Docs Signed","Loan Funded","Notes","Closer"]
    : baseHeaders;
  return (
